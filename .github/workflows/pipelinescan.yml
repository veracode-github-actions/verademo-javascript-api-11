  ...
  veracode-scan:
    ...
    steps:
      ...
      - name: Veracode Pipeline Scan
        ...
        run: |
          ...
          for file in verascan/*; do
            if [ -f "$file" ]; then
              echo "Scanning $file..."
              ./veracode static scan "$file" --policy-file "Al-Test.json" --results-file "results.json" --filtered-json-output-file || true
            fi
          done
      ...
      - name: Move filtered results to verascan folder
        run: |
          mkdir -p verascan
          cp filtered-results.json verascan/filtered_results.json
